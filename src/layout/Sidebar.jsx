...
import {
  ...
  ListItemButton, ListItemIcon, ListItemText, Switch, TextField,
  Tooltip, Typography,
} from "@mui/material";
...
function SectionLabel({ children }) {
  return (
    <Typography sx={{
      px:2, pt:1.75, pb:0.5, display:"block",
      fontWeight:900, fontSize:10, letterSpacing:1.3,
...
function RowWithPlus({ icon, label, selected, onClick, onPlus }) {
  return (
    <Box sx={{
      mx:1, mb:0.25, borderRadius:1.5, display:"flex", alignItems:"center",
      borderLeft: selected ? `3px solid ${ACCENT}` : "3px solid transparent",
      bgcolor: selected ? SEL_BG : "transparent",
      "&:hover": { bgcolor: selected ? SEL_BG : HOVER_BG },
      transition:"all 0.12s",
      ...
      <ListItemButton onClick={onClick} sx={{ borderRadius:1.5, px:1.5, py:0.7, flexGrow:1 }}>
        ...
        <ListItemText primary={label} primaryTypographyProps={{
          fontSize:13.5, fontWeight:selected?800:500,
          color:selected?ACCENT:"text.primary",
        }} />
      </ListItemButton>
      <Tooltip title={`Add ${label}`} placement="right">
        <IconButton size="small" onClick={onPlus} sx={{
          mr:0.75, width:24, height:24, opacity:0.4,
  ...
  return (
    <>
      <ListItemButton onClick={onToggle} sx={{
        mx:1, mb:0.25, borderRadius:1.5, px:1.5, py:0.7,
        borderLeft: selected ? `3px solid ${ACCENT}` : "3px solid transparent",
        bgcolor: selected ? SEL_BG : "transparent",
        "&:hover":{ bgcolor: selected ? SEL_BG : HOVER_BG },
        transition:"all 0.12s",
        ...
        {open
          ? <ExpandMoreIcon sx={{ fontSize:17, opacity:0.45 }} />
          : <ChevronRightIcon sx={{ fontSize:17, opacity:0.45 }} />}
      </ListItemButton>
...
export default function Sidebar({ drawerWidth }) {
  ...
  useEffect(() => {
    window.api.getSettings()
      .then(s => {
        setIspName(s?.isp_name || "");
        setPosEnabled(s?.pos_enabled !== "0");
      })
      .catch(() => {});
  }, []);

  // Reload companies whenever user navigates — picks up newly added companies
  useEffect(() => {
    window.api.listCompanies?.()
      .then(d => setCompanies(d || []))
      .catch(() => {});
  }, [p]); // re-run on every route change
  ...
  return (
    <Drawer variant="permanent" sx={{
      ...
      {/* ── Header ─────────────────────────────────────────────────────────── */}
      <Box sx={{
        px: 2.5, height: 64, minHeight: 64, flexShrink: 0,
        ...
        <Box sx={{
          width: 38, height: 38, borderRadius: 2, overflow: "hidden", flexShrink: 0,
        ...
        <Box sx={{ overflow:"hidden", WebkitAppRegion:"no-drag" }}>
          <Typography sx={{
            color:"#fff", fontWeight:900, fontSize:15.5, lineHeight:1.2,
            whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis",
          }}>
            {ispName || "Cedar ISP"}
          </Typography>
          <Typography sx={{
            color:"rgba(255,255,255,0.65)", fontSize:11.5, mt: 0.25,
            whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis",
          }}>
      ...
      <Box sx={{ overflowY:"auto", overflowX:"hidden", flexGrow:1, py:0.75 }}>
        ...
        {/* Overview — gated by dashboard permission */}
        {can("dashboard") && <>
          ...
          <List disablePadding>
            {can("payments_view") && (
              <RowWithPlus icon={<PaymentsIcon />} label="Payments"
                selected={isPaymentsRoute}
                onClick={() => nav("/payments/list")}
                onPlus={can("payments_create") ? () => nav("/payments/create") : null} />
            )}

            {can("drawer_view") && (
              <NavItem icon={<PointOfSaleIcon />} label="Cash Drawer"
                to="/drawer" selected={isDrawerRoute} />
            )}
        ...
        {(can("users_view") || can("whatsapp")) && <>
          ...
          <List disablePadding>
            ...
            {can("whatsapp") && (
              <NavItem
                icon={<WhatsAppIcon sx={{ color:isWhatsAppRoute?"#25D366":undefined }} />}
        ...
        {/* POS — only when enabled */}
        {posEnabled && <>
          <SectionLabel>Point of Sale</SectionLabel>
          <List disablePadding>
            <NavItem icon={<StorefrontIcon />} label="Terminal" to="/pos"
              selected={isPosRoute && !isPosItems && !isPosSales && !isPosDrawer} />
            <NavItem icon={<SellIcon />} label="Items & Stock" to="/pos/items" selected={isPosItems} />
            <NavItem icon={<LocalMallIcon />} label="Sales" to="/pos/sales" selected={isPosSales} />
            <NavItem icon={<PointOfSaleIcon2 />} label="POS Drawer" to="/pos/drawer" selected={isPosDrawer} />
          </List>
          <NavDivider />
        </>}
        ...
        {isAdmin && (
          <CollapseNav icon={<SettingsIcon />} label="System"
            ...
            {can("activity") && (
              <NavItem icon={<HistoryIcon />} label="Activity Log" to="/activity"
                selected={isActivityRoute} indent />
            )}
            {can("archive") && (
              <NavItem icon={<InventoryIcon />} label="Archive" to="/archive"
                selected={isArchiveRoute} indent />
            )}
            <NavItem icon={<BadgeIcon />} label="Employees" to="/employees"
              selected={is("/employees")} indent />
            {can("settings") && (
              <NavItem icon={<BusinessIcon />} label="Settings" to="/settings/isp"
                selected={isSettingsRoute} indent />
            )}
          </CollapseNav>
        )}
      ...
      <Box sx={{ px:1.25, py:1.25, borderTop:"1px solid rgba(0,0,0,0.07)", flexShrink:0 }}>
        ...
        <Box sx={{ display:"flex", gap:0.75 }}>
          <Box onClick={() => window.api.appRestart?.()} sx={{
            flex:1, display:"flex", flexDirection:"column", alignItems:"center",
            gap:0.4, py:0.9, borderRadius:1.5, cursor:"pointer",
            bgcolor:"rgba(25,118,210,0.06)", border:"1px solid rgba(25,118,210,0.13)",
            "&:hover":{ bgcolor:"rgba(25,118,210,0.13)", borderColor:ACCENT },
            transition:"all 0.12s",
          }}>
          ...
          <Box onClick={() => setEmployee(null)} sx={{
            flex:1, display:"flex", flexDirection:"column", alignItems:"center",
            gap:0.4, py:0.9, borderRadius:1.5, cursor:"pointer",
            bgcolor:"rgba(211,47,47,0.05)", border:"1px solid rgba(211,47,47,0.13)",
            "&:hover":{ bgcolor:"rgba(211,47,47,0.12)", borderColor:"error.main" },
            transition:"all 0.12s",
          }}>
