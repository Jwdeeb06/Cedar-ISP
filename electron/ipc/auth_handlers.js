...
function registerAuthHandlers(ipcMain, db) {
    ...
    return new Promise((resolve, reject) => {
      db.get(
        ...
        (err, row) => {
          ...
          if (!row) return resolve({ ok: false, reason: "INVALID_CREDENTIALS" });
          if (row.password !== password)
            return resolve({ ok: false, reason: "INVALID_CREDENTIALS" });
    ...
    return new Promise((resolve, reject) => {
      db.run(
        ...
        function(err) {
          ...
          logAction(db, { action:"ADD_EMPLOYEE", entity:"employees", entity_id:this.lastID,
  ...
  ipcMain.handle("update-employee", async (event, p) => {
    ...
    if (!is_active) {
      const adminCount = await new Promise((res, rej) =>
        db.get(`SELECT COUNT(*) AS c FROM employees WHERE role='admin' AND is_active=1 AND id!=?`,
  ...
  ipcMain.handle("change-password", async (event, { id, old_password, new_password }) => {
    ...
    if (old_password && emp.password !== old_password)
      return { ok: false, reason: "WRONG_PASSWORD" };

    return new Promise((resolve, reject) => {
      db.run(`UPDATE employees SET password=? WHERE id=?`, [new_password, id],
  ...
  ipcMain.handle("delete-employee", async (event, id) => {
    ...
    const hasActivity = await new Promise((res, rej) =>
      db.get(`SELECT COUNT(*) AS c FROM activity_log WHERE actor=
